  ...
  sections: [
    {
      ...
      title: "소개",
      paragraphs: [
        "20여 개 글로벌 공급사를 연동하는 호텔 예약 시스템에서 응답 속도와 대용량 배치를 개선했습니다. 800만 건 규모의 콘텐츠 동기화 배치를 유량 제어로 3주에서 5일로 줄이고, 140여 대 IDC 서버의 실시간 관제 체계를 구축했습니다.",
          ...
          projects: [
            {
              ...
              summary:
                "AX를 위한 AI 인프라 구축을 담당했습니다. 개발 과정과 의사결정을 기록하는 AI 워크스페이스 표준을 설계해 팀 내 지식베이스를 구축하고, 코드베이스 검색·로그 검색·DB 조회·통계 조회 등 사내 리소스에 접근 가능한 MCP와 스킬을 개발했습니다. 인증 게이트와 개별 토큰 발급 체계를 설계하여 자격증명 관리와 감사가 가능하도록 했으며, 이렇게 구축한 인프라를 개인 로컬 하네스용 플러그인, Slack·Jira 기반 챗봇 등 다양한 형태로 전사에 제공하여 조직의 AI 활용 기반을 마련했습니다.",
              achievements: [
                {
                  text: "MCP 개발",
                  items: [
                ...
                "OpenBao OIDC 기반 인증 게이트 및 개별 토큰 발급으로 사내 MCP 접근 자격증명 관리·Audit 체계 구축",
                "로컬 하네스 플러그인, Slack·Jira 챗봇 등 다중 채널로 전사 배포하여"
              ],
            },
            {
              id: "hotel-booking",
              ...
              summary:
                "아고다 등 20여 개 호텔 공급사와 카카오, 현대이지웰 등 8개 이상의 제휴사를 연결하는 호텔 예약 시스템의 개발·운영 전반과 최적화를 담당했습니다. 요금 조회 요청마다 20여 개 공급사에서 fanout 방식으로 요금을 가져와 최저가를 연산해 응답하는 구조라, 단일 요청의 처리 비용이 커 응답 속도를 확보하는 것이 핵심 과제였습니다. 또한 공급사당 최대 265만 개에 달하는 호텔 콘텐츠를 운영 영향 없이 배치로 동기화하고, IDC 기반 140여 대 서버의 관제 체계를 구축해 시스템의 성능과 안정성을 개선했습니다.",
              achievements: [
                {
                  text: "AIMD 기반 유량 제어로 800만 건 API 통신 규모의 호텔 콘텐츠 배치를 약 3주에서 5일로 단축 — 실효 동시성 2 → 5.66, 실패율 0.007%",
